...
/**
 * Registry of addon extension hooks. The two {@link #register(IFoodRuleProvider)} and
 ...
 * which provider wins.
 *
 * <p><b>Neutral when empty:</b> both lists are {@code new CopyOnWriteArrayList<>()} initialized empty,
 * with no static initializer and no self-registration anywhere in the base mod. Only addons and
 * tests call the registration entry points. With no addon present every hook-query loop is empty and falls
 * through to the built-in behavior, leaving behavior unchanged when no addon is present.
 *
...
public final class IZombieExtensions {
     ...
     * ({@link AttackerDecision}); their wiring is deferred, so no handler queries this list yet.
     *
     * <p>Addons must not call this method, modify the returned list, retain its reference, or depend on its
     * implementation type.
     *
