...
/**
 * Pure, Minecraft-free data form of the explicit food table. Each row's buffs and debuffs are a list of
 ...
 *
 * <p>Resolution is per effect, not per row in bulk: each effect's own config keys are read and it
...
final class ZombieFoodTable {
    ...
    record Row(
        ...
        private static <T> List<T> resolve(
            ...
            // Freeze this group as soon as it is complete: List.copyOf makes it immutable and throws NPE
    ...
    // ---------- The 42-row explicit table (identical ids, order, and values to the previous EXPLICIT map) ----------
