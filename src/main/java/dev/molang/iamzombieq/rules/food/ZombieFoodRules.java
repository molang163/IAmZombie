...
public final class ZombieFoodRules {
    ...
    // (the not-loaded fallback lives in that resolver unchanged; see ZombieFoodEvents). ----------
    ...
    /**
     * Id-only resolution (no ItemStack, so no tag layer) for the tooltip caller. Resolves via the explicit map,
    ...
    /**
     * ItemStack-aware resolution for the events layer, running the full catch-all (the mod tier tags and
    ...
    public static FoodRule ruleForStack(
        ...
        // Mod tier tags in the catch-all path. Built on demand so this class stays loadable without Minecraft on the
    ...
    // live value, else the registered default). Only the location of the read changed; the per-key
